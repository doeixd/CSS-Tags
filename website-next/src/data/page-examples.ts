...
const examples: Record<string, PageExampleDefinition> = {
    ...
    "components/view-transition": {
        title: "Click between same-document views",
        description: "Native hash links trigger the shipped router; shared transition names connect the avatar and heading between states.",
        preview: `<div data-view-transitions class="example-panel"><nav aria-label="Profile views"><layout-cluster gap="var(--space-sm)"><a data-view-trigger href="#profile-overview">Overview</a><a data-view-trigger href="#profile-activity">Activity</a></layout-cluster></nav><section data-view-page id="profile-overview" active tabindex="-1"><layout-cluster align="center" gap="var(--space-sm)"><div style="view-transition-name:profile-avatar;display:grid;place-items:center;inline-size:3rem;block-size:3rem;border-radius:50%;background:var(--accent);color:var(--text-on-accent)">A</div><div><h3 style="view-transition-name:profile-title">Avery Gray</h3><p class="example-muted">Workspace administrator</p></div></layout-cluster></section><section data-view-page id="profile-activity" tabindex="-1"><layout-cluster align="center" gap="var(--space-sm)"><div style="view-transition-name:profile-avatar;display:grid;place-items:center;inline-size:4rem;block-size:4rem;border-radius:var(--radius-md);background:var(--secondary);color:var(--text-on-secondary)">A</div><div><h3 style="view-transition-name:profile-title">Recent activity</h3><p class="example-muted">Invited two teammates · 4 minutes ago</p></div></layout-cluster></section></div>`,
        code: `<div data-view-transitions>\n  <nav aria-label="Profile views">\n    <a data-view-trigger href="#profile-overview">Overview</a>\n    <a data-view-trigger href="#profile-activity">Activity</a>\n  </nav>\n  <section data-view-page id="profile-overview" active>\n    <div class="profile-avatar">A</div><h2>Avery Gray</h2>\n  </section>\n  <section data-view-page id="profile-activity">\n    <div class="profile-avatar profile-avatar--square">A</div><h2>Recent activity</h2>\n  </section>\n</div>\n<style>\n  .profile-avatar { view-transition-name: profile-avatar; }\n  h2 { view-transition-name: profile-title; }\n</style>`,
    },
    "core/base": {
    ...
    "js/view-transition": {
        title: "Switch views with explicit button targets",
        description: "Buttons can target views with `to`; the initializer updates active state, history, ARIA, focus, and animation together.",
        preview: `<div data-view-transitions class="example-panel"><layout-cluster gap="var(--space-sm)"><button data-view-trigger to="js-summary" type="button">Summary</button><button data-view-trigger to="js-details" type="button">Details</button></layout-cluster><section data-view-page id="js-summary" active tabindex="-1"><h3>Summary</h3><p>Focus moves here after the transition update.</p></section><section data-view-page id="js-details" tabindex="-1"><h3>Details</h3><p>This target does not require a URL hash.</p></section></div>`,
        code: `import { initializeViewTransitions } from "css-tags/view-transition.js";\n\n<div data-view-transitions>\n  <button data-view-trigger to="summary" type="button">Summary</button>\n  <button data-view-trigger to="details" type="button">Details</button>\n  <section data-view-page id="summary" active>...</section>\n  <section data-view-page id="details">...</section>\n</div>\n\ninitializeViewTransitions(document);`,
    },
