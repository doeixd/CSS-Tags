...
`@latest` is convenient while experimenting, but production sites should pin a commit or release so an upstream change cannot alter a deployed page unexpectedly:

```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/doeixd/CSS-Tags@COMMIT_OR_TAG/index.css">
```

There is not currently an npm package. If your build imports local CSS, clone or vendor the repository and import its root entry point:

```css
@import url("./vendor/css-tags/index.css");
```
