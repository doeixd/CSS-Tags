...
## Exportable Theme Layer

The documentation Theme creator generates a complete drop-in stylesheet:
...
The separate layer is intentionally loaded after the library's declared layer
order. It therefore overrides authored input tokens without `!important`.
Choose the named-theme export to target both a root and a subtree with
`data-theme="name"`.
