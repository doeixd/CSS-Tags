...
Use `--font-size-prose`, `--line-height-prose`, and `--measure-prose` for
long-form content; `--font-size-ui` and `--font-size-control` for application
chrome; and `--font-weight-heading` plus `--letter-spacing-heading` for heading
character. Font families have body, heading, UI, and code inputs.
