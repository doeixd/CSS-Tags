...
- `--space-heading-to-content`
- `--space-heading-to-lead`
- `--space-lead-to-content`
...
- `--space-blockquote-margin-inline`
...
`--space-flow` controls the normal sibling rhythm. `--space-rich-block` controls
the shared vertical rhythm for code blocks, figures, blockquotes, rules, and
callouts. Relationship tokens then handle the places where unlike elements
should sit closer together, without creating separate top and bottom variables
...
:root {
  ...
  --space-lead-to-content: 1.2em;

  --code-block-bg: var(--surface-overt);
