    ...
    <div class="example-panel">
      <layout-cluster gap="var(--space-sm)" justify="space-between">
        ...
        <layout-cluster gap="var(--space-xs)">
          <badge role="subtle">This week</badge>
...
The current layout selectors primarily support the custom-element form shown above. `layout-stack` additionally exposes `[data-layout-stack]` and `.layout-stack` for the `.stack-intrinsic` child hook, but those aliases do not yet receive the complete host layout rules. Use custom layout tags for reliable behavior today.
