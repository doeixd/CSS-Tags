...
Variants set the button-local `--bg` and `--fg` aliases: `.btn-primary`,
`.btn-secondary`, `.btn-success`, `.btn-warning`, and `.btn-error`. For direct
theme customization, prefer `--form-button-background` and
`--form-button-color`; `.form-button` isolates those values from an ancestor's
